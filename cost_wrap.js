/**
 * PURE PHYSICS ENGINE: Vehicle Wraps
...
function calculateWrap(inputs, data) {
    ...
    inputs.panels.forEach(p => {
        ...
        totalSqFt += area;
        let retailUnit = p.material.startsWith('perf') ? retPerf : retWrap;
        
        if(p.included) {
            R(`Panel: ${p.label} (Included)`, 0, `Included in primary wrap`);
        } else {
            R(`Panel: ${p.label} [${p.material === 'wrap' ? 'Cast' : 'Window Perf'}]`, retailUnit * area, `${area.toFixed(1)} SF @ $${retailUnit}`);
...
window.WRAP_CONFIG = {
    ...
    retails: [ { key: 'Retail_Price_Vehicle_SqFt', label: 'Wrap Base ($/SF)' }, { key: 'Retail_Price_Perf_SqFt', label: 'Perf Base ($/SF)' } ],
