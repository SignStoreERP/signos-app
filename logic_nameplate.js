/**
 * PURE PHYSICS ENGINE: ADA Etch Nameplates (v2.2)
...
function calculateNameplate(inputs, data) {
    ...
    } else if (inputs.mat.Series.includes('Ultra')) {
        baseRateSqIn = parseFloat(data.Retail_Price_Ultra_116 || 0.65);
        if (inputs.mat.Thickness && inputs.mat.Thickness.includes('1/8')) {
            baseRateSqIn = parseFloat(data.Retail_Price_Ultra_18 || 0.85);
        }
    } else {
        baseRateSqIn = parseFloat(data.Retail_Price_Mattes_116 || 0.55);
    }

    let retailPrint = (sqin * baseRateSqIn) * inputs.qty;
    let t1Qty = parseFloat(data.Tier_1_Qty || 10);
    
    if (inputs.qty >= t1Qty) {
        retailPrint *= (1 - parseFloat(data.Tier_1_Disc || 0.05));
    }

    // Setup Fee intentionally zeroed out for UI
    ...
    return {
        retail: { unitPrice: grandTotal / inputs.qty, printTotal: retailPrint, setupFee: feeSetupRetail, grandTotal: grandTotal, isMinApplied: grandTotalRaw < minOrder, baseRate: baseRateSqIn },
        cost: { total: subTotal + riskBuffer, breakdown: { rawSubstrate: costSubstrate, rawPaint: paintMatCost, costPrepress: costPrepress, costHandling: costLoad, costMachine: costMachRun, costPaintLabor: costPaintLabor, runHrs: (engraveMins / 60) } },
