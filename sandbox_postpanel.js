...
function printSandbox() {
    ...
    geom.panels.forEach((p, i) => {
        ...
        let hB = p.hBraces || 0;
        if (vB > 0) {
            const spc = (sPw - ft*2) / (vB + 1);
            for (let b = 1; b <= vB; b++) { svgFrontSolid += `<rect x="${panelX + ft + (spc * b) - ft/2}" y="${vY}" width="${ft}" height="${vHeight}" fill="#94a3b8"/>`; }
        }
        if (hB > 0) {
            const spc = (sPh - ft*2) / (hB + 1);
            for (let b = 1; b <= hB; b++) { svgFrontSolid += `<rect x="${fX}" y="${currY + ft + (spc * b) - ft/2}" width="${sFw}" height="${ft}" fill="#94a3b8"/>`; }
        }
    ...
    geom.panels.forEach((p, i) => {
        ...
        if (vB > 0) {
            const spc = (sPw - ft*2) / (vB + 1);
            for (let b = 1; b <= vB; b++) { svgFrontXray += `<rect x="${panelX + ft + (spc * b) - ft/2}" y="${vY}" width="${ft}" height="${vHeight}" fill="#94a3b8"/>`; }
        }
        if (hB > 0) {
            const spc = (sPh - ft*2) / (hB + 1);
            for (let b = 1; b <= hB; b++) { svgFrontXray += `<rect x="${fX}" y="${currYx + ft + (spc * b) - ft/2}" width="${sFw}" height="${ft}" fill="#94a3b8"/>`; }
        }
